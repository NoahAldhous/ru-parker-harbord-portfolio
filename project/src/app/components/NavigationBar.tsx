    ...
    <>
      <div className="hidden sm:flex flex-row w-screen h-navbar-height top-0 left-0 overflow-x-hidden fixed z-2000 px-6 sm:px-page-horizontal-padding justify-between items-center bg-gradient-to-b from-white via-white to-transparent">
        <Link
          onClick={handleClose}
          key={"/"}
          className="whitespace-nowrap transition-all duration-600 opacity-90 md:hover:opacity-100 z-20 w-2/3"
          href={"/"}
        >
          <motion.h1
        ...
        >
          {navBarItems.map((item) => (
            ...
            >
              ...
              {pathname === item.href ? (
                <motion.div
                  className="bottom-0 inset-x-0 sm:h-px md:h-px lg:h-0.5 bg-black dark:bg-primary rounded-lg z-0 w-full"
                  layoutId="underline"
                />
              ) : null}
            </Link>
      ...
      >
        ...
        <section className="flex flex-row justify-end items-center w-1/3">
          <button
            onClick={() => setNavigationModalOpen(!navigationModalOpen)}
            className="p-0 pr-0.5 text-right font-superRetro italic text-xs align-middle items-center overflow-x-visible"
          >
            Menu
          </button>
        </section>
